...
侧边导航按「评委叙事主线」收敛为**七页主导航**（收敛不删功能，所有原页面保留为分组子项）：

1. **数据驾驶舱**（系统概览 / 数据源管理 / 测试评估）
2. **岗位发现与演化**（岗位管理 / JD解析 / 新岗位发现 / 岗位能力更新 / 能力演化）
3. **能力图谱大脑**（能力图谱 / 图谱探索）
4. **简历与匹配诊断**（个人中心·候选人管理 / 简历解析 / 匹配分析 / 数字人面试官）
5. **学习路径**（求职者）
6. **审核与可信治理**（人工审核 / 系统设置）
7. **项目展示**（评委/云展厅入口）

账号设置移至右上角用户菜单；菜单按角色过滤，某分组只剩一个子页时自动折叠为单入口。
